...
int RealInfoCache::UpdateRealInfoLatestCache(std::string stock_code,
                                             std::string value) {
  int rc = 0;
  ...
  engine->SetValue(real_key.c_str(), real_key.length(), value.c_str(),
                   value.length());
  static_cast<base_storage::RedisStorageEngineImpl*>(engine)->ExpireValue(
      real_key.c_str(), 30);
  return rc;
}
...
int RealInfoCache::GetRealInfoLatestCache(std::string stock_code,
                                          std::string *value) {
  int rc = 0;
  base_dic::KunDicPool *pool = base_dic::KunDicPool::GetInstance();
  base_dic::AutoDicCommEngine dic(pool, pool_key_);
